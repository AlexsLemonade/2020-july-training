...
```{r}
# Remove somethings from the global environment before moving on to other 
# methods
...
In the GSEA notebook (`02-gene_set_enrichment_analysis`), we used GSEA preranked. 
...
Here's a screenshot of how we could use these files on GenePattern:
...
(To look just at the pathways for the Group 3 digoxin vs. no treatment, we could do `topTable(fit2, coef = 1)`, instead.)

```{r}
topTable(fit2, number = 10)
```

It can be helpful to visualize these pathways individually, with something like a jitter plot rather than a heatmap.

```{r}
data.frame(gsva_score = gsva_results["HALLMARK_TGF_BETA_SIGNALING", ],
           subtype_treatment) %>%
  ...
  ggplot2::ggtitle("HALLMARK_TGF_BETA_SIGNALING")
```

```{r}
data.frame(gsva_score = gsva_results["HALLMARK_INTERFERON_GAMMA_RESPONSE", ],
           subtype_treatment) %>%
  ggplot2::ggplot(ggplot2::aes(x = subtype_treatment,
                               y = gsva_score)) +
  ggplot2::geom_jitter(width = 0.1, alpha = 0.5) +
  ...
  ggplot2::ggtitle("HALLMARK_INTERFERON_GAMMA_RESPONSE")
```

```{r}
data.frame(gsva_score = gsva_results["HALLMARK_G2M_CHECKPOINT", ],
           subtype_treatment) %>%
  ggplot2::ggplot(ggplot2::aes(x = subtype_treatment,
                               y = gsva_score)) +
  ggplot2::geom_jitter(width = 0.1, alpha = 0.5) +
  ...
  ggplot2::ggtitle("HALLMARK_G2M_CHECKPOINT")
```

```{r}
data.frame(gsva_score = gsva_results["HALLMARK_HEDGEHOG_SIGNALING", ],
           subtype_treatment) %>%
  ggplot2::ggplot(ggplot2::aes(x = subtype_treatment,
                               y = gsva_score)) +
  ggplot2::geom_jitter(width = 0.1, alpha = 0.5) +
  ...
  ggplot2::ggtitle("HALLMARK_HEDGEHOG_SIGNALING")
```
